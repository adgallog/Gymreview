...
def call_api_predic():
    url = 'http://127.0.0.1:8000/predict_gender/'
    ...
    payload = {
        ...
        'Workout_Frequency' : int(Workout_Frequency),
    ...
    if st.button("Hacer Predicción"):
        ...
        prediccion = response.json()['prediction']
        if response.status_code == 200:
            st.write(f"Los datos ingresados pertenecen a una persona del Genero: **{prediccion}**")
        else:
